...
A `ggplot()` command is typically built using sub-commands linked by `+` symbols. The sub-commands are executed one-after-another to build the final plot. The core components are:  

* **a data set** - can be a data frame, or a spatial shapefile  
* **coordinate system** - the x and y axes can reflect the range of values within specified variables
* **geoms** - geometric marks to visualize the data (e.g. points, lines, etc)  
* **aesthetics** - linking attributes such as color and size of geoms to variables in the data set  

TODO: IMAGE describing parts of a template ggplot command  

{{% notice tip %}}
A command beginning with `ggplot()` will print the visualization but not save it. Using the assignment operator `<-` in the first line will assign the visualization to an R object and save it for later use.
{{% /notice %}}  

### Geospatial mapping  

In the Place section of the template, `ggplot()` is used to create geospatial maps of attack rates and other indicators. In these cases, the coordinate system reflects latitude/longitude, and `ggplot()` uses `geom_sf()` to accept and display points, polygons, or lines shapefiles.


### Aesthetic themes and labels  

In the templates, detailed aesthetics parameters are managed by creating theme objects that can be referenced multiple times.  
...
Likewise, this chunk establishes the settings for labels. The settings for x-axis and y-axis labels, title, and subtitle are stored in the `epicurve_labels` object, which is referenced in several epidemic curve `ggplot()` commands.  
...
A **for loop** is used to repeat or iterate code over each value in a vector. The vector can be a sequence of values - including the values within a data frame variable. 
...
* **Setup**, including :  
     ...
     * Parentheses containing:  
          * A placeholder term (e.g. "i", "value", or another short word)  
          * The keyword `in`  
          * The name of the vector to cycle through  
     * Opening bracket `{`  

* **Actions** and commands to be performed for each item in the vector  
     * If the item is referenced in a command, use the placeholder term instead.  
     
* **Closing** - A closing bracket `}`
      

A basic example **for loop** is below. `number_list` is defined as a vector of four numbers. The **for loop** is set-up to examine each item ("i") in `number_list`. The only command in the **for loop** is to print the value of "i" plus 2. Finally, the brackets are closed. See that one output is printed for each item in `number_list`.
