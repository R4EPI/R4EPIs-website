...
Here, we show how `%in%` can be used to find which random sample of 10 letters are in the set of (a, b, c):

```{r}
# randomly sample 10 letters from the alphabet
set.seed(99) 
my_letters <- sample(letters, 10) 
my_letters

# use %in% to ask "which letters are within the set of (a, b, c)?"
abc <- my_letters %in% c("a", "b", "c") 

# the result is a logical (TRUE/FALSE) vector
abc

# this vector can be used to subset our original vector, returning everything that is TRUE.
my_letters[abc]
```
