...
Here, ggplot() is used to create the map. Remember that ggplot() subcommands must be connected by plus symbols at the end of each line (+) (except the last line). Read more about ggplot2 and ggplot() in the R Basics pages.  
...
**scale_fill_brewer()** uses a color palatte from orange to red ("OrRd"), and specifies a title for the legend. If you want a different color scale, use this code from the package RColorBrewer to see the possible options, and replace "OrRd" with the desired code:  
