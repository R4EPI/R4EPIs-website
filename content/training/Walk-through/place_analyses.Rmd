...
{{% notice tip %}}
There are many quartiers without population denominators that will make the graph difficult to read. Thus, we first filter `ar` to remove those using the opposite (`!`) of the `is.na()` function (see Operators R Basics page). Note that if this step is added, any quartiers with missing denominators will not appear in the graph.
{{% /notice %}}
...
3) In the last command, `mutate()` is used to create the categorical variable within 'ar', using `breakers` to define the breaks in the numeric variable in `ar`, which is `AR (per 10,000)`. 
...
**A "for loop"" is used to iterate over (perform the same actions to) every value in a vector.** In this case, we want to make the map and bar chart for each region. You can read more about for loops in the Advanced R Basics pages, or [this tutorial](https://www.datamentor.io/r-programming/for-loop/).  
