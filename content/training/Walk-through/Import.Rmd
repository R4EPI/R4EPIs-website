...
**In this template section you choose the appropriate code chunk to import your dataset.**  

For this exercise our dataset is a **non-standardized Excel file**, so we use the `read_nonDHIS_data` code chunk. **The linelist Excel file can be downloaded from the case study overview page.** 
...
Either delete or comment out (#) all lines of code in the other data import chunks (`read_fake_data` and `read_DHIS_excel_data`). 
...
linelist_raw <- rio::import(here("AJS_AmTiman.xlsx"), which = "linelist")
```
