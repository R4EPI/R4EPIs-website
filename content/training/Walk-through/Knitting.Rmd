...
TODO: Graphic showing knitting  

**Producing the final RMarksdown report is called "knitting"**. When you are editing the RMarkdown script, you will see a special option to "Knit"" at the top of the script, near the save shortcut button. Clicking "Knit" will begin the process of creating the final document. The option to "Knit" does not appear for normal R scripts, only RMarkdown scripts. Note the following:  
...
It is also possible to write in-line code - this is where in the script text you put a placeholder for a number or output that is calculated from your data. Examples of this are found throughout the template. To write in-line code, in the white space put an R command that returns the desired number within backticks (` `). 
