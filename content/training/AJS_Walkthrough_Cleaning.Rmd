...
2) **Set date variables to class Date**
...
3) **Re-code values in patient_facility_type**  
...
We can modify a chunk from the cleaning section of the template. The code below re-defines linelist_cleaned as itself  mutated such that in the case when it's value equals "Oui", the value is changed (~) to "Inpatient", and so on.  

As you modify this chunk for your own situation, remember to include commas at the end of the case_when() lines, and that each left-hand side (LHS) of the ~ must be a logical statement, not just a value. You can read more about case_when() here.  
...
Skip number of days under observation bc no date of exit variable exists
...
Create DIED variable, but recognize that the values in the dataset are in French.
...
**Maybe keep only the inpatient observations? And Add an "inpatient" variable to those that have "date_of_admission" value?**  
...
# Filter linelist_cleaned to keep observations where date_of_onset is after April 2016 AND before October 2017 OR missing
linelist_cleaned <- linelist_cleaned %>%
  filter((date_of_onset >= as.Date("2016-04-01") & date_of_onset <= as.Date("2017-10-01")) | date_of_onset) | 
